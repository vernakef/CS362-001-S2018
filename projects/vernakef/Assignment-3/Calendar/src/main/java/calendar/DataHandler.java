    ...
    private static GregorianCalendar getNextApptOccurrence(Appt appt, 
        ...
        switch (appt.getRecurBy()) {
                ...
                for (int k = 0; k < 7; k++) {
                    ...
                    for (int i = 0; i < recurDays.length; i++) {
                        ...
                        if (recurDays[i] == newDayOfWeek) {
                            return nextDay;  //had to set back to [i] instead of [k] as bug was causing seg fault too often
                        }
                    }
                }
            ...
            case Appt.RECUR_BY_YEARLY:
                ...
                nextDay.add(nextDay.MONTH, 1);  //new bug to replace old bug, YEAR --> MONTH
                return nextDay;
        }
        return null;
    }
