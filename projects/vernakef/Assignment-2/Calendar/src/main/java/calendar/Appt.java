...
public class Appt{
    ...
    public boolean isOn(int day, int month, int year) {
        return (day == getStartDay() && month == getStartMonth() 
                && year == getStartYear());  //new bug, year --> month/ getStartYear --> getStartMonth
    }
    ...
    public boolean hasTimeSet() {
        return (getStartHour() != NO_TIME); //had to change this bug back in order to obtain high branching percentage in CalDay's getFullInformationApp() function
    } 
    /**
